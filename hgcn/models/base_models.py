...
class NCModel(BaseModel):
    ...
    def compute_metrics(self, embeddings, data, split):
        ...
        loss = cb_loss(data['labels'][idx], output, self.beta, self.gamma, data['labels'][idx].unique(return_counts=True)[1].tolist())
        acc, f1, recall, conf_mat = acc_f1(output, data['labels'][idx], split, average=self.f1_average)
        metrics = {'loss': loss, 'acc': acc, 'f1': f1, 'recall': recall, 'conf_mat':conf_mat}

        if split=='test':
            with open("preds.pkl","wb") as f:
                pickle.dump((output,[data['node_labels'][i] for i in idx]),f)
            #print(f1,recall)
